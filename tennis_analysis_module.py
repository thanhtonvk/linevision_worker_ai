...
class TennisAnalysisModule:
    ...
    def __init__(self, ball_model_path="ball_best.pt", person_model_path="yolov8m.pt", pose_model_path="yolov8n-pose.pt"):
    ...
    def analyze_video(self, video_path, ball_conf=0.7, person_conf=0.6, 
                     angle_threshold=50, intersection_threshold=100, 
        ...
        direction_flags, person_detections_old = self.ball_detector.get_enhanced_direction_change_flags(
            frames, ball_positions, angle_threshold=angle_threshold, 
            person_conf=person_conf, intersection_threshold=intersection_threshold
        )
        
        # 5. Person tracking và pose estimation
        print("Đang tracking người và phân tích pose...")
        person_detections, pose_detections = self.person_tracker.detect_and_track_persons(
            frames, ball_positions, direction_flags
        )
        
        # 6. Phân tích kỹ thuật tennis
        technique_analysis = self.person_tracker.analyze_tennis_technique(person_detections, court_bounds)
        ...
        highest_speed_info = self._find_highest_speed_hit(
            frames, ball_positions, person_detections, direction_flags, fps, court_bounds
        )
        
        # 7.2. Tính toán danh sách người chơi hay nhất
        best_players = self._calculate_best_players(
            frames, ball_positions, person_detections, technique_analysis, fps, court_bounds
        )
        ...
        visualization_video_path = self._create_visualization_video(
            frames, ball_positions, direction_flags, person_detections, 
            pose_detections, highest_speed_info, best_players, match_statistics, fps
        )
    ...
    def _find_highest_speed_hit(self, frames, ball_positions, person_detections, 
                                direction_flags, fps, court_bounds):
        """
            ...
            if direction_flags[frame_idx] == 2:  # Bóng được đánh bởi người
                ...
                if velocity > max_velocity:
                    ...
                    if person_info:
                        ...
                        person_hits = self.person_tracker.ball_hits_by_person.get(person_id, [])
                        ...
                        for hit in person_hits:
                            ...
                            if pose is not None:
                                ...
                                # Tính góc khụy gối
                                if conf[11] > 0.5 and conf[13] > 0.5 and conf[15] > 0.5:  # Left side
                                    left_hip = keypoints[11]
                                    left_knee = keypoints[13]
                                    left_ankle = keypoints[15]
                                    left_knee_angle = self._calculate_angle_3points(left_hip, left_knee, left_ankle)
                                    
                                    if conf[12] > 0.5 and conf[14] > 0.5 and conf[16] > 0.5:  # Right side
                                        right_hip = keypoints[12]
                                        right_knee = keypoints[14]
                                        right_ankle = keypoints[16]
                                        right_knee_angle = self._calculate_angle_3points(right_hip, right_knee, right_ankle)
    ...
    def _calculate_best_players(self, frames, ball_positions, person_detections, 
                                 technique_analysis, fps, court_bounds):
        """
        ...
        for person_id, stats in person_stats.items():
            ...
            in_court_ratio = stats['hits_in_court'] / stats['total_hits'] if stats['total_hits'] > 0 else 0
            ...
            score = self._calculate_player_score(
                in_court_ratio, avg_ball_speed, stats['total_hits'], 
                avg_shoulder_angle, avg_knee_bend_angle
            )
    ...
    def _calculate_player_score(self, in_court_ratio, avg_ball_speed, total_hits, 
                                avg_shoulder_angle, avg_knee_bend_angle):
        """
        ...
        total_score = in_court_score + speed_score + hits_score + shoulder_score + knee_score
        
        return total_score
    
    def _calculate_match_statistics(self, ball_positions, direction_flags, 
                                    technique_analysis, fps):
        """
    ...
    def _create_visualization_video(self, frames, ball_positions, direction_flags, 
                                    person_detections, pose_detections, 
                                    highest_speed_info, best_players, match_statistics, fps):
        """
        ...
        for frame_idx, frame in enumerate(frames):
            ...
            if frame_idx < len(ball_positions) and ball_positions[frame_idx] != (-1, -1):
                x, y = int(ball_positions[frame_idx][0]), int(ball_positions[frame_idx][1])
                ...
                if highest_speed_info['frame'] == frame_idx:
                    cv2.rectangle(vis_frame, (x-15, y-15), (x+15, y+15), (0, 255, 255), 3)
                    cv2.putText(vis_frame, "MAX SPEED", (x+20, y), 
            ...
            # Add frame info
            cv2.putText(vis_frame, f"Frame: {frame_idx}", (width - 150, 30), 
